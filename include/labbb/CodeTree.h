...
public:
  ...
  virtual bool operator==(const LTLNode &other) {
    ...
    case HISTORICALLY:
      const HISTORICALLYNode *otherHistoricallyNode =
          dynamic_cast<const HISTORICALLYNode *>(&other);
      const HISTORICALLYNode *thisHistoricallyNode =
          dynamic_cast<const HISTORICALLYNode *>(this);
      return thisHistoricallyNode == otherHistoricallyNode;
    case EXISTS:
      const EXISTSNode *otherExistsNode =
          dynamic_cast<const EXISTSNode *>(&other);
      const EXISTSNode *thisExistsNode = dynamic_cast<const EXISTSNode *>(this);
      return thisExistsNode == otherExistsNode;
    case FORALL:
      const FORALLNode *otherForallNode =
          dynamic_cast<const FORALLNode *>(&other);
      const FORALLNode *thisForallNode = dynamic_cast<const FORALLNode *>(this);
      return thisForallNode == otherForallNode;
    case IMPLIES:
      const IMPLIESNode *otherImpliesNode =
          dynamic_cast<const IMPLIESNode *>(&other);
      const IMPLIESNode *thisImpliesNode =
          dynamic_cast<const IMPLIESNode *>(this);
    ...
    case ATOM_REF:
      const ATOM_REFNode *otherAtomRefNode =
          dynamic_cast<const ATOM_REFNode *>(&other);
      const ATOM_REFNode *thisAtomRefNode =
          dynamic_cast<const ATOM_REFNode *>(this);
...
public:
  ...
  size_t hash() { return ::hash({op, child->id}); }
  bool operator==(const NOTNode &other) { return child->id == other.child->id; }
};
...
public:
  LTLNode *left;
  LTLNode *right;
  ANDNode(LTLNode *left, LTLNode *right)
      : LTLNode(AND), left(left), right(right) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() {
...
public:
  LTLNode *left;
  LTLNode *right;
  ORNode(LTLNode *left, LTLNode *right)
      : LTLNode(OR), left(left), right(right) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() {
    int small = std::min(left->id, right->id);
    int big = std::max(left->id, right->id);
...
public:
  ...
  LTLNode *right;
  time_id_s time;
  SINCENode(LTLNode *left, LTLNode *right, time_id_s time)
      : LTLNode(SINCE), left(left), right(right), time(time) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() {
...
public:
  ...
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, child->id}); }
  bool operator==(const PRENode &other) { return child->id == other.child->id; }
};
...
public:
  LTLNode *child;
  time_id_s time;
  ONCENode(LTLNode *child, time_id_s time)
      : LTLNode(ONCE), child(child), time(time) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, time.from, time.to, child->id}); }
  bool operator==(const ONCENode &other) {
    return child->id == other.child->id && time.from == other.time.from &&
           time.to == other.time.to;
  }
};
...
public:
  LTLNode *child;
  time_id_s time;
  HISTORICALLYNode(LTLNode *child, time_id_s time)
      : LTLNode(HISTORICALLY), child(child), time(time) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, time.from, time.to, child->id}); }
...
public:
  ...
  EXISTSNode(LTLNode *child, std::vector<std::string> vars)
      : LTLNode(EXISTS), child(child), vars(vars) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, child->id}); }
...
public:
  LTLNode *child;
  std::vector<std::string> vars;
  FORALLNode(LTLNode *child, std::vector<std::string> vars)
      : LTLNode(FORALL), child(child), vars(vars) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, child->id}); }
...
public:
  LTLNode *left;
  LTLNode *right;
  IMPLIESNode(LTLNode *left, LTLNode *right)
      : LTLNode(IMPLIES), left(left), right(right) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() { return ::hash({op, left->id, right->id}); }
...
public:
  std::string var;
  std::string value;
  ATOMNode(std::string var, std::string value)
      : LTLNode(ATOM), var(var), value(value) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() {
  ...
  bool operator==(const ATOMNode &other) { return var == other.value; }
};
...
public:
  ...
  ATOM_REFNode(std::string var, std::string reference)
      : LTLNode(ATOM_REF), var(var), reference(reference) {}
  std::variant<DdNode *, bool> operate();
  size_t hash() {
    std::hash<std::string> hash_fn;
    int hash_var = hash_fn(var);
  ...
  bool operator==(const ATOM_REFNode &other) { return var == other.reference; }
};
